classdef affine_flow
    ...
    properties (Dependent)
        ...
        % Smoothing constant for rectilinear smoothing
        %
    ...
    methods         % public computational methods
        ...
        function a = advance(a)
            ...
            % array of images and we want a cell array of flow fields
            % results:
            %
    ...
    methods (Access = private)
        ...
        function a = affine_flowxy(a)
            ...
            if isempty(a.smth1)
                a.smth1 = gsmooth2(a.im1, a.sigmaxy, false, ...
                    regionl + [-1 1 -1 1]);
            end
            if isempty(a.smth2)
                a.smth2 = gsmooth2(a.im2, a.sigmaxy, false, ...
                    regionl + [-1 1 -1 1]);
            end
            ...
            if a.sampstep > 1
                ...
                flowargs = cellfun(@(z) {a.sample(z, steps)}, flowargs);
            end
    ...
    methods (Static, Access = private)
        ...
        function affineparams = solvexy(x, y, gx, gy, gt)
            % AFFINESOLVE first order optic flow parameters -rectilinear
            %   AFFINEPARAMS = AFFINESOLVE(X, Y, GXS, GYS, GTS) returns the
            %   affine flow parameters vx0, vy0, d, r, s1, s2, as a struct
            %   with those f ields. The inputs are the x and y sample
            %   positions and the x, y and t gradients, all as column
            %   vectors.
...
%             Rsquared = 1- SSerr / SStot
        ...
        function affineparams = solverw(rmin, nw, r, w, gr, gw, gt)
            % AFFINE_SOLVERW first order optic flow parameters - log-polar
            %   AFFINEPARAMS = AFFINE_SOLVERW(RMIN, NW, R, W, GR, GW, GT)
